...
class User extends CI_Controller{
    ...
    public function createAkun(){
        ...
        if(!$_POST){
            ...
            $data->no_reff = str_replace('.','', $this->input->post('no_reff'));
    ...
    public function cetakLaporanAkun()
    {
        $data = [
        ...
        //this->mypdf->generate('user/laporan');
        $this->pdf->setpaper('A4', 'portrait');
        $this->pdf->filename = "Laporan Akun";
        $this->pdf->load_view('laporan/akun', $data);
    }

     public function cetakNeracaSaldo(){

        $jurnalumum = $this->db->select('*');
        ...
        $this->db->from('transaksi');
        $this->db->where('jenis_saldo','debit');
        $debit = $this->db->get()->result_array();

        $this->db->select_sum('saldo');
        $this->db->from('transaksi');
        $this->db->where('jenis_saldo','kredit');
        $kredit = $this->db->get()->result_array();
        ...
        $this->pdf->setpaper('A4', 'portrait');
        $this->pdf->filename = "Laporan Neraca Saldo";
        $this->pdf->load_view('laporan/neracasaldo', $data);
    }
    ...
    public function cetakJurnalUmum(){
        ...
        $this->pdf->setpaper('A4', 'portrait');
        $this->pdf->filename = "Laporan Jurnal Umum";
        $this->pdf->load_view('laporan/jurnalumum', $data);
    }

    public function cetakBukuBesar($no_reff){
        $noReff = $no_reff;

        $bukubesar = $this->db->select('*');
                    $this->db->from('transaksi');
                    $this->db->join('akun', 'akun.no_reff = transaksi.no_reff');
                    $this->db->where('transaksi.no_reff', $noReff);
        $query = $this->db->get()->result();
        ...
        $this->pdf->setpaper('A4', 'portrait');
        $this->pdf->filename = "Laporan Buku Besar";
        $this->pdf->load_view('laporan/bukubesar', $data);
    }
